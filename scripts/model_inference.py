...
class SimpleGCN(torch.nn.Module):
    ...
    def forward(self, data, return_hidden_embeddings=False):
        ...
        x = self.lin(x)
        x_pre_lin = x
        if return_hidden_embeddings:
            return F.log_softmax(x, dim=1), x_pre_lin
        else:
...
# Inference function
def infer_model(landmarks, return_hidden_embeddings=False):
    """"
    ...
    with torch.no_grad():
        ...
        # Perform inference
        if return_hidden_embeddings:
            out, hidden_embeddings = model(data, return_hidden_embeddings=True)
            pred = out.argmax(dim=1)
            return inverse_label_mapping[pred.cpu().numpy()[0]], hidden_embeddings
        else:
            out = model(data)
            pred = out.argmax(dim=1)
...
if __name__ == '__main__':
    ...
    test_predictions, hidden_embeddings = infer_model(ilg.reference_landmarks, return_hidden_embeddings=True)
    print(test_predictions, hidden_embeddings[0].tolist())
