...
with mp.solutions.face_mesh.FaceMesh(refine_landmarks=True) as face_mesh:
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            expression, hidden_embeddings = infer_model(normalized_landmarks, return_hidden_embeddings=True)
            hidden_embeddings = hidden_embeddings[0].detach().numpy()  # Convert tensor to numpy array
        else:
            expression = 'None'
            hidden_embeddings = []
        ...
        # frame = utils.draw_embeddings(frame, hidden_embeddings)
